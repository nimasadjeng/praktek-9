...
while ($row = mysqli_fetch_array($sql)) {
    ...
    $sheet->setCellValue('D'.$i,$row['tglmasuk']);
    $sheet->setCellValue('E'.$i,$row['nis']);
    $sheet->setCellValue('F'.$i,$row['nopesujian']);
    $sheet->setCellValue('G'.$i,$row['appaud']);
    $sheet->setCellValue('H'.$i,$row['aptk']);
...
while ($row = mysqli_fetch_array($sql)) {
    ...
    $sheet->setCellValue('Y'.$i,$row['namakel']);
    $sheet->setCellValue('Z'.$i,$row['kec']);
    $sheet->setCellValue('AA'.$i,$row['kodepos']);
    $sheet->setCellValue('AB'.$i,$row['ttinggal']);
    ...
    $sheet->setCellValue('AJ'.$i,$row['namaneg']);
    $i++;
}
$sql = mysqli_query($conn,"SELECT * FROM dataayah");
$i  = 5;
$no = 1;
while ($row = mysqli_fetch_array($sql)) {
    $sheet->setCellValue('AK'.$i,$row['namaayah']);
    $sheet->setCellValue('AL'.$i,$row['tlayah']);
    $sheet->setCellValue('AM'.$i,$row['pendayah']);
    $sheet->setCellValue('AN'.$i,$row['pekerayah']);
    $sheet->setCellValue('AO'.$i,$row['salaryayah']);
    $sheet->setCellValue('AP'.$i,$row['berkebayah']);
    $i++;
}
$sql = mysqli_query($conn,"SELECT * FROM dataibu");
$i  = 5;
$no = 1;
while ($row = mysqli_fetch_array($sql)) {
    $sheet->setCellValue('AQ'.$i,$row['namaibu']);
    $sheet->setCellValue('AR'.$i,$row['tlibu']);
    $sheet->setCellValue('AS'.$i,$row['pendibu']);
    $sheet->setCellValue('AT'.$i,$row['pekeribu']);
    $sheet->setCellValue('AU'.$i,$row['salaryibu']);
    $sheet->setCellValue('AV'.$i,$row['berkebibu']);
    $i++;
}
